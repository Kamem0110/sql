...
-- SEMANA 3 // Relatório de senioridade
...
-- SEMANA 3 // Listar faixa etária dos dependentes
...
--SEMANA 3 // Relatório de plano de saúde
...
SELECT
    ...
    CASE
        WHEN C.DEPARTAMENTO = 'SEOPE' THEN C.SALARIO * 0.01
        WHEN C.DEPARTAMENTO = 'SEDES' THEN C.SALARIO * 0.02
        WHEN C.DEPARTAMENTO = 'DEPTI' THEN C.SALARIO * 0.03
        WHEN C.DEPARTAMENTO = 'DIR' THEN C.SALARIO * 0.05
    END
    +
    CASE
        WHEN MONTHS_BETWEEN(SYSDATE, DEPENDENTE.DATA_NASCIMENTO) >= 215 THEN 100.00
        WHEN DEPENDENTE.PARENTESCO = 'CÃ´njuge' THEN 100.00
        else 25.00 
    END as valor_mensalidade
    FROM
        COLABORADOR C
    JOIN
        DEPENDENTE ON C.MATRICULA = DEPENDENTE.COLABORADOR;
